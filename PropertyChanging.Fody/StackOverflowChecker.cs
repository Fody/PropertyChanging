...
public class StackOverflowChecker
{
    ...
    void Process(IEnumerable<TypeNode> notifyNodes)
    {
        foreach (var node in notifyNodes)
        {
            ...
            {
                if (node.EventInvoker.IsBefore)
                {
                    if (CheckIfGetterCallsSetter(propertyData.PropertyDefinition))
                    {
                        throw new WeavingException(string.Format("{0} Getter calls setter which will cause a stack overflow as the setter uses the getter for obtaining the before values.", propertyData.PropertyDefinition.GetName()));
                    }

                    if (CheckIfGetterCallsVirtualBaseSetter(propertyData.PropertyDefinition))
                    {
                        throw new WeavingException(string.Format("{0} Getter of calls virtual setter of base class which will cause a stack overflow as the setter uses the getter for obtaining the before values.", propertyData.PropertyDefinition.GetName()));
                    }
                }
            }
